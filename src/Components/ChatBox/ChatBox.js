...
export default function ChatBox({
  ...
  useEffect(() => {
    ...
    const getuserData = async () => {
      try {
        fetch(`${configData.apiurl}/user/getuser/${userId}`)
          .then((resp) => resp.json())
          .then((resp) => {
  ...
  useEffect(() => {
    ...
    if (chat && receiveMessage !== null && receiveMessage.chatId === chat._id) {
    ...
    <>
      <div className="ChatBox-container">
        {chat ? (
          ...
          <span style={{ color: "white" }} className="chatbox-empty-message">
            {" "}
